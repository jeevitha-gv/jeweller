...
class dashboard{
  //--------------------------------------------------------------------------------------------------  
    ...
    public function status($companyId){
        $sql = 'SELECT a.status, count(*) AS count FROM audit a,company c WHERE a.company_id=c.id and c.id=? and a.status is not null GROUP BY status';
         $paramArray=array($companyId);
          $dbOps=new DBOperations();
        return $dbOps->fetchData($sql,'i',$paramArray);
    ...
    public function noOfAudits($companyId){
        $sql='SELECT count(*) as count FROM audit a,company c WHERE a.company_id=c.id and c.id=?';
        $paramArray=array($companyId);
          $dbOps=new DBOperations();
        return $dbOps->fetchData($sql,'i',$paramArray);
     ...
     public function noOfAuditsPublished($companyId){
      ...
      public function noOfAuditsDue($companyId){
        $sql='SELECT count(*) as count FROM audit a,company c WHERE a.company_id=c.id and c.id=? and (a.status!="published" and a.status!="approved")';
        $paramArray=array($companyId);
          $dbOps=new DBOperations();
        return $dbOps->fetchData($sql,'i',$paramArray);
    ...
    public function noOfAuditsDelayed($companyId){
        $sql='SELECT count(*) as count FROM audit a,company c WHERE a.company_id=c.id and c.id=7 and a.end_date>CURDATE() and a.status!="published" and a.status!="approved"';
        $paramArray=array($companyId);
          $dbOps=new DBOperations();
        return $dbOps->fetchData($sql,'i',$paramArray);
    ...
    public function auditRemainderChartCompl($companyId,$complianceId){
        $sql='SELECT ac.priority,count(*) as count FROM audit a,audit_clauses ac WHERE (a.status != "published" or a.status!="approved") AND a.company_id=?  AND ac.audit_id=a.id  AND a.compliance_id=? GROUP BY ac.priority';
    ...
    {
       $sql='SELECT `id`, `title`, `start_date`, `end_date`,monthname(start_date) AS startmonth,monthname(end_date) AS endmonth,year(start_date) AS startyear,year(end_date) AS endyear FROM `audit` GROUP BY `title`,`startmonth` ORDER BY `start_date`';
        return $this->fetchDataFromDB($sql);  
    }

    //--------------------Configurable Dasboard-------------------//
    public function chartTypes(){
        $sql='SELECT `id`, `name` FROM `audit_chart_type` ';
        return $this->fetchDataFromDB($sql);
    }
...
//Audit Related Queries Ends Here --------------------------------------------------------//
   ...
   /* public function events(){
    ...
    public function getAllTask($projectId){
        $sql = 'SELECT t.task_name as taskName,t.project_id as projectId,t.id as taskId,t.assigned_to as assignedTo,t.due_date as dueDate,u.id as userId,u.last_name as userName,t.description as description FROM task t, user u WHERE t.assigned_to=u.id and project_id=?';
        $paramArray = array();
    ...
    public function riskScoring(){
     ...
     public function riskLocation(){
        $sql='SELECT b.name, count(r.location) AS count FROM risks r, bu_location b WHERE r.location=b.id GROUP BY r.location';
        $dbOps=new DBOperations();
        return $dbOps->fetchData($sql);
    ...
    public function getNoOfCreatedRisk(){
        $sql = 'SELECT r.status as name,count(r.status) AS count FROM risks r WHERE r.status="create"';
       $dbOps=new DBOperations();
        return $dbOps->fetchData($sql);         
    }
    public function getNoOfMitigatedRisk(){
        $sql = 'SELECT r.status as name,count(r.status) AS count FROM risks r WHERE r.status="mitigated"';
       $dbOps=new DBOperations();
        return $dbOps->fetchData($sql);         
    }
    public function getNoOfReviewedRisk(){
        $sql = 'SELECT r.status as name,count(r.status) AS count FROM risks r WHERE r.status="reviewed"';
       $dbOps=new DBOperations();
        return $dbOps->fetchData($sql);         
    }
    public function getTotalNoOfRisks(){
        $sql = 'SELECT count(*) AS total_records FROM risks r';
       $dbOps=new DBOperations();
        return $dbOps->fetchData($sql);         
    }
  ...
  public function AssetGroup(){
    ...
    public function AssetClassfication(){
     ...
     public function Assetlocation(){
        $sql='SELECT al.name,count(a.location_id) AS count FROM asset a,bu_location al WHERE a.location_id=al.id GROUP BY a.location_id';
        $dbOps=new DBOperations();
        return $dbOps->fetchData($sql);
...
//     
    ...
    public function getAllSupportTickets(){
        $sql='SELECT ua.id,u.last_name,ua.logged_in_time,ua.logged_out_time FROM `user_activity_log` ua,user u,role r,user_role ur WHERE ua.user_id=u.id and u.id=ur.user_id and r.id=ur.role_id and (r.id=7 or r.id=4) ORDER BY ua.id desc';
        $dbOps=new DBOperations();
        return $dbOps->fetchData($sql);
    }
...
// bcpm Dashboard
    ...
    public function getRiskNotify(){
        $sql='SELECT id,status,subject,created_date FROM `risks` ORDER BY `id` DESC';
        $dbOps=new DBOperations();
        return $dbOps->fetchData($sql);
    }
    
    public function getAuditNotify(){
        $sql='SELECT id,status, title, created_date FROM `audit` ORDER BY `id` DESC';
        $dbOps=new DBOperations();
        return $dbOps->fetchData($sql);
    }

    public function getPolicyNotify(){
        $sql='SELECT id,status, title, created_date FROM `policy` ORDER BY `id` DESC';
        $dbOps=new DBOperations();
        return $dbOps->fetchData($sql);
    }
}